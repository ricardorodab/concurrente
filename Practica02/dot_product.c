...
int main (int argc, char *argv[])
{
  NUMTHRDS = 4;
  VECLEN = 100000;
  double aleatorio = -3.141592;
  if(argc >= 3){
    int entrada1 = atoi(argv[1]);
    int entrada2 = atoi(argv[2]);
    if(entrada1 == 0){
      printf("\n");
      exit(1);
    }
    ...
    if(argc >= 4){
      int entrada3 = atoi(argv[3]);
      srand48(entrada3);
    }
    NUMTHRDS = entrada1;
    VECLEN = entrada2;
    callThd = malloc(sizeof(pthread_t)*NUMTHRDS);
  }
